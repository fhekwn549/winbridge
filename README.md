...
**현재 상태: P2A POC 검증 완료** — `./install.sh`로 무인 자동 설치 (~30~50분), 카톡 자동 시작, Rust 트레이/런처, 키보드 입력, 양방향 클립보드, 폰 페어링까지 end-to-end 동작. 프로젝트명은 가제입니다.

---

## What

A bash-driven installer provisions a headless **Windows Server 2022 Evaluation** VM under libvirt/KVM. A Rust host manager then launches **KakaoTalk** through an embedded RDP viewer so it behaves like a small Linux app window. The guest keeps the standard `explorer.exe` shell (한글 IME 호환에 필수) but hides the taskbar and desktop icons — so the Linux desktop sees a near-clean KakaoTalk window.
...
- Linux 호스트는 Rust `winbridge` 매니저로 VM을 깨우고 KakaoTalk 전용 RDP 창을 띄움
...
8. Rust 매니저 또는 FreeRDP 관리 창으로 카톡 단독 표시 확인 + 폰 페어링

이후 재접속:

```bash
./target/release/winbridge                  # 트레이 실행
./target/release/winbridge start --mode app # KakaoTalk 앱 창 직접 실행
```
...
│   └─ embedded RDP viewer + clipboard ──────────────────▶ [VM]  │
│                                                                 │
...
- **bash 설치 + Rust 사용 경로.** 설치/VM 생성은 멱등 bash 스크립트가 담당하고, 일상 사용은 Rust 매니저가 담당
- **데이터 영속성은 VM qcow2 내부.** virtiofs 호스트 공유는 P2A에 없음 (P2B 후보)
- **수동 단계는 manual-checks 절차로 격리.** ISO URL/sha256은 사용자가 직접 확보

## Native manager (Rust)

P2A 설치(`./install.sh`) 이후, Rust로 작성된 호스트 매니저로 카톡을 일반 앱처럼 사용할 수 있다.

```bash
# build prerequisites
sudo apt install -y libgtk-4-dev libgraphene-1.0-dev libpango1.0-dev libvirt-dev pkg-config libssl-dev
...
./target/release/winbridge start                  # VM 깨우기 + KakaoTalk 앱 모드 RDP 창
./target/release/winbridge start --mode app       # 기본값과 동일, 480x680 KakaoTalk 창
./target/release/winbridge start --mode app --display experimental-multimon  # 실험용 RDP 다중 모니터 경로
./target/release/winbridge start --mode desktop   # Windows 전체 관리용 desktop mode RDP 창
./target/release/winbridge start --mode desktop --display experimental-multimon  # 2모니터 관리용 실험 경로
./target/release/winbridge stop                   # VM managed save
./target/release/winbridge status                 # VM 상태 출력
```

GNOME 사용자는 system tray 표시를 위해 `gnome-shell-extension-appindicator` 설치가 필요할 수 있다.

자격증명은 P2A의 `~/.config/winbridge/credentials` 파일을 그대로 사용한다.

`app` mode는 KakaoTalk 전용 480x680 RDP 세션을 열고, GTK 창 제목/애플리케이션 ID/desktop entry를 `KakaoTalk`로 맞춘다. 키보드 입력과 텍스트 클립보드는 embedded RDP viewer에서 처리한다. `--display experimental-multimon`은 Windows RDP DisplayControl 기반 다중 모니터 분리를 검증하기 위한 별도 실험 경로이며, 일반 KakaoTalk 실행에는 기본 `stable-slots`를 사용한다. Windows 설정/업데이트/디버깅이 필요하면 `--mode desktop`을 사용한다.

현재 VM에서 작업표시줄 자동 숨김을 다시 적용해야 하는 경우 `scripts/windows/position-kakaotalk.ps1` 내용을 Windows PowerShell에서 실행한다. 새 VM 설치 경로는 `config/firstboot.ps1`에서 같은 설정을 자동 적용한다.

## Quick start

전제: Ubuntu 22.04 + KVM 활성 + FreeRDP(`xfreerdp` 또는 `xfreerdp3`) + `libvirt-daemon-system`/`virt-install`/`genisoimage`/`qemu-utils` 설치, 8 GB+ RAM, 50 GB+ 여유 디스크.
...
- libvirt `qemu:///system`
- FreeRDP 2/3 (`xfreerdp` 또는 `xfreerdp3`, 설치 검증/관리용)
...
- D-Bus 알림 브리지, 뱃지, 전역 핫키 없음 (P2B)
- virtiofs 데이터 영속성/호스트 공유 없음 (P2B)

## Roadmap

- **P2A (현재):** Server 2022 Eval 무인 설치 자동화, explorer 셸 + 작업표시줄 숨김 폴백, Rust 트레이/런처, KakaoTalk 앱 창, 키보드 입력, 양방향 클립보드
- **P2B (예정):** VM 자동 시작/idle 관리/만료 관리, 알림 브리지, 데이터 영속성 (virtiofs), 추가 앱 후보
