...
# 선택적으로 virtio-win ISO도 다운로드한다 (QEMU guest agent 설치용).
...
: "${WINBRIDGE_VIRTIO_ISO_URL:=https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/virtio-win.iso}"
: "${WINBRIDGE_VIRTIO_ISO_SHA256:=}"
: "${WINBRIDGE_VIRTIO_ISO_DEST:=$HOME/.cache/winbridge/virtio-win.iso}"
...
환경변수 (선택):
  ...
  WINBRIDGE_ENABLE_QEMU_GA 1이면 virtio-win ISO도 다운로드 (기본 0)
  WINBRIDGE_VIRTIO_ISO_URL qemu-ga용 virtio-win ISO URL
  WINBRIDGE_VIRTIO_ISO_SHA256 virtio-win ISO sha256 (선택, 설정 시 검증)
  WINBRIDGE_VIRTIO_ISO_DEST virtio-win ISO 저장 경로
USAGE
}
...
elif [ ! -f "$SENTINEL" ] && verify_sha256; then
...
[ -z "$WINBRIDGE_VIRTIO_ISO_URL" ] && {
    log_error "WINBRIDGE_VIRTIO_ISO_URL 미설정"
    exit 1
}
require_cmd curl "sudo apt install -y curl"
mkdir -p "$(dirname "$WINBRIDGE_VIRTIO_ISO_DEST")"
VIRTIO_SENTINEL="$WINBRIDGE_SENTINEL_DIR/01-virtio-iso-downloaded.done"

verify_virtio_sha256() {
    [ -f "$WINBRIDGE_VIRTIO_ISO_DEST" ] || return 1
    if [ -z "$WINBRIDGE_VIRTIO_ISO_SHA256" ]; then
        return 0
    fi
    local actual
    actual=$(sha256sum "$WINBRIDGE_VIRTIO_ISO_DEST" | cut -d' ' -f1)
    [ "$actual" = "$WINBRIDGE_VIRTIO_ISO_SHA256" ]
}

if [ -f "$VIRTIO_SENTINEL" ] && verify_virtio_sha256; then
    log_info "virtio-win ISO 이미 존재, skip ($WINBRIDGE_VIRTIO_ISO_DEST)"
    exit 0
fi

if [ ! -f "$VIRTIO_SENTINEL" ] && verify_virtio_sha256; then
    log_info "virtio-win ISO 존재. sentinel 생성 후 skip ($WINBRIDGE_VIRTIO_ISO_DEST)"
    touch "$VIRTIO_SENTINEL"
    exit 0
fi

log_info "virtio-win ISO 다운로드 시작 ($WINBRIDGE_VIRTIO_ISO_URL → $WINBRIDGE_VIRTIO_ISO_DEST)"
VIRTIO_PART="${WINBRIDGE_VIRTIO_ISO_DEST}.part"
if curl -fL --retry 3 --retry-delay 5 -C - -o "$VIRTIO_PART" "$WINBRIDGE_VIRTIO_ISO_URL"; then
    mv "$VIRTIO_PART" "$WINBRIDGE_VIRTIO_ISO_DEST"
else
    log_error "virtio-win ISO 다운로드 실패"
    rm -f "$VIRTIO_PART"
    exit 1
fi

if ! verify_virtio_sha256; then
    actual=$(sha256sum "$WINBRIDGE_VIRTIO_ISO_DEST" | cut -d' ' -f1)
    log_error "virtio-win sha256 불일치"
    log_error "  expected: $WINBRIDGE_VIRTIO_ISO_SHA256"
    log_error "  actual:   $actual"
    rm -f "$WINBRIDGE_VIRTIO_ISO_DEST"
    exit 1
fi

touch "$VIRTIO_SENTINEL"
log_info "virtio-win ISO 다운로드 완료 ($WINBRIDGE_VIRTIO_ISO_DEST)"
