...
Linux 앱 설치 스크립트는 `winbridge`를 `~/.local/bin/winbridge`에 넣고 KakaoTalk 런처와 로그인 자동시작 항목을 등록합니다.

```bash
scripts/host/08-install-linux-app.sh
```

런처는 `winbridge start --mode app`을 실행하므로 KakaoTalk 아이콘 클릭만으로 VM 시작 또는 재개 후 KakaoTalk을 열 수 있습니다.
